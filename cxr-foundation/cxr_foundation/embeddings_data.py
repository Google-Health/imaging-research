...
def read_record_example(filename: str) -> tf.train.Example:
  """
...
def extract_embedding_values(example_data: tf.train.Example) -> np.ndarray:
  """
  Extract the embeddings values contained in an Example object, extracted from a file
  generated by the CXR foundation service.
  ...
  try:
    values = np.array(example_data.features.feature[constants.EMBEDDING_KEY].float_list.value, dtype='int32')
    return values
  except ValueError:
    print(f"Input Example does not contain expected CXR Foundation embedding structure.")
    raise

def read_record_embeddings(filename: str) -> np.ndarray:
  """
  Read the embeddings values contained in a TFRecord embedding file, generated by the CXR
  foundation service.
  ...
  """
  return extract_embedding_values(read_record_example(filename))


def parse_embedding_from_example(
